...
class ThermoMLDataset(InMemoryDataset):
    ...
    def process(self):
        nrow = self.nrow
        cols = ["mlc1", "mlc2", "TK", "PPa", "phase", "type", "m"]
        ...
        for raw_path in self.raw_paths:
            ...
            for datapoint in progressbar(
                dataframe.iter_rows(named=True), desc="data points", total=total
            ):
...
class ParametersDataset(InMemoryDataset):
    ...
    def __init__(
        self, root, transform=None, pre_transform=None, pre_filter=None, Notebook=True
    ):
        self.Notebook = Notebook
        super().__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self):
        ...
        for row in progressbar(pure.iter_rows(), desc="datapoint", total=pure.shape[0]):
            ...
            try:
                ...
                graph.y = torch.tensor(
                    (row[2:] + row[2:] + (0, 0, 0)), dtype=torch.float
                )
                graph.c1 = row[0]
                graph.c2 = row[0]
                datalist.append(graph)
        ...
        for row in progressbar(kij.iter_rows(), desc="datapoint", total=kij.shape[0]):
            ...
            try:
                graph = BinaryGraph(inchi1, inchi2)
                graph.y = torch.tensor(
                    (para1 + para2 + (k_ij, 0, 0)), dtype=torch.float
                )
                graph.c1 = row[4]
                graph.c2 = row[3]
                datalist.append(graph)
